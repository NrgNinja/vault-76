...
#         avg_gen_time = round(df["Gen&Sort"].mean(), 3)
#         avg_write_time = round(df["Writing"].mean(), 3)
                 
#         plt.bar(threads.index(thread_num) - bar_width, avg_gen_time, width=bar_width, color="#cfe2f3ff", edgecolor="grey", label="Generation")
#         plt.bar(threads.index(thread_num) + bar_width, avg_write_time, width=bar_width, color="#e01e20", edgecolor="grey", label="Writing")

#         plt.text(threads.index(thread_num) - bar_width, avg_gen_time + 0.1, avg_gen_time, ha="center", va="bottom")
#         plt.text(threads.index(thread_num) + bar_width, avg_write_time + 0.1, avg_write_time, ha="center", va="bottom")
