...
// // Serializes records into binary and stores them in a file on disk
// pub fn store_hashes(buckets: &DashMap<[u8; 7], Vec<Record>>, filename: &str, num_threads: usize) -> io::Result<()> {
//     let file = Arc::new(Mutex::new(File::create(filename)?));
//     let record_size = std::mem::size_of::<Record>();

//     // Collect the vectors from the DashMap into a Vec
//     let vectors: Vec<Vec<Record>> = buckets.iter().map(|entry| entry.value().clone()).collect();

//     vectors.par_iter().enumerate().try_for_each(|(prefix_idx, records)| {
//         let chunk_size = std::cmp::max(records.len() / num_threads, 1);

//         records.chunks(chunk_size).enumerate().try_for_each(|(chunk_idx, chunk)| {
//             let mut buffer = Vec::with_capacity(chunk.len() * record_size);
//             for record in chunk {
//                 buffer.extend_from_slice(&record.nonce);
//                 buffer.extend_from_slice(&record.hash);
//             }

//             let start_pos = ((prefix_idx * records.len()) + (chunk_idx * chunk_size)) as u64 * record_size as u64;
//             let file_clone = Arc::clone(&file);

//             let mut file_lock = file_clone.lock().unwrap();
//             file_lock.seek(SeekFrom::Start(start_pos))?;
//             file_lock.write_all(&buffer)?;
//             file_lock.flush()?;
